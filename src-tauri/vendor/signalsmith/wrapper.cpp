...
void signalsmith_stretch_set_formant_base(signalsmith_stretch_t *handle, float frequency) {
    // CORRIGE PAR RAPPORT A L'AMONT. Le wrapper du crate `signalsmith-stretch` appelle ici
    // setFormantSemitones(frequency), ce qui n'est pas la base de formants mais le decalage --
    // et donc ECRASE le reglage pose juste avant. Un appel a set_formant_base(0) apres
    // set_formant_factor_semitones(-3) remettait le decalage a zero sans rien dire.
    handle->instance.setFormantBase(frequency);
}
