...
unsafe extern "C" {
    fn signalsmith_stretch_create_preset_default(channel_count: c_int, sample_rate: c_float) -> *mut Handle;
    ...
    fn signalsmith_stretch_set_transpose_factor_semitones(handle: *mut Handle, semitones: c_float, tonality_limit: c_float);
    fn signalsmith_stretch_set_formant_factor_semitones(handle: *mut Handle, semitones: c_float, compensate_pitch: c_int);
    fn signalsmith_stretch_set_formant_base(handle: *mut Handle, frequency: c_float);
    fn signalsmith_stretch_exact(handle: *mut Handle, input: *mut c_float, input_length: usize, output: *mut c_float, output_length: usize) -> bool;
...
impl Stretch {
    pub fn new(sample_rate: u32) -> Self {
        let handle = unsafe { signalsmith_stretch_create_preset_default(1, sample_rate as c_float) };
    ...
    pub fn set(&mut self, pitch_semitones: f32, formant_semitones: f32) {
        unsafe {
    ...
    pub fn run(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        unsafe {
            signalsmith_stretch_exact(self.handle, source.as_mut_ptr(), source.len(), body.as_mut_ptr(), body.len());
        }
