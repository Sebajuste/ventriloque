# pack-builder — faire tourner la recette d'un paquet sur une copie installée d'un jeu

Un binaire, embarqué dans Ventriloque et posé dans `engine\` au premier lancement. Il porte les
...
| **`storage/rdar.rs`** | le lecteur d'archives REDengine, écrit ici — Cyberpunk 2077 |
| **`storage/wwise.rs`** | le Wwise Vorbis rendu lisible, portage de ww2ogg |
| **`voice-assembly`** | le décodage Ogg Vorbis et le montage, partagés avec `assemble-voice` |
| **Rhai** | le moteur de script, et son bac à sable |

```powershell
pack-builder --script <recette.rhai> --game <racine du jeu> --out <dossier de travail>
pack-builder --list <fragment> --game <racine du jeu>      # pour fouiller à la main
pack-builder --hash <chemin depot>                       # un chemin Cyberpunk -> son FNV1a64
```
...
```rhai
product()                     le nom de code du jeu ouvert : "s2", "fenris", "cp77"
list(motif)                 les entrées dont le nom concorde, en minuscules
size(nom)                   la taille d'une entrée, en octets
text(nom)                    le contenu texte d'une entrée
log(ligne)                   une ligne de journal, que la fenêtre affiche
voice(#{ nom, prises, duree, min, max })        monte et écrit une référence
character(#{ id, nom, univers, voix, repliques })  écrit une fiche de personnage
```
...
Les segments Oodle existent ailleurs dans le jeu ; `storage/rdar.rs` les refuse au lieu de faire semblant.
...
hexadécimaux, et `--hash` traduit un chemin pour qui écrit une recette.
...
Vorbis valide auquel il manque sa configuration. `storage/wwise.rs` la remet, bit pour bit.
...
- **L'application ne sait pas trouver le jeu.** `games\` reconnaît une installation par son
  ...
  dans `storage/` pour le fabricant, pas côté application. Le joueur désigne son dossier.
...
- **`tools\extract-casc\` et `tools\assemble-voice\` sont absorbés** par ce binaire, qui fait les
  deux. Ils restent en place, et `--list` remplace le premier.
