...
$ici = $PSScriptRoot
$casclib = Join-Path (Split-Path -Parent $ici) "pack-builder\vendor\CascLib"
$obj = Join-Path $ici "obj"
$exe = Join-Path $ici "extraire-casc.exe"

if (-not (Test-Path (Join-Path $casclib "sources-cpp.cpp"))) {
    throw "CascLib absente. Depuis tools\pack-builder : git clone --depth 1 https://github.com/ladislav-zezula/CascLib.git vendor\CascLib"
}
...
Write-Output ""
Write-Output "tools\extract-casc\extraire-casc.exe — $poids Mo"
