...
| `..\monter-voix\` | décode les `.ogg`, en assemble une référence de clonage `.wav` |
...
git clone --depth 1 https://github.com/ladislav-zezula/CascLib.git outils\extraire-casc\vendor\CascLib
powershell -File outils\extraire-casc\build.ps1
cargo build --release --manifest-path outils\monter-voix\Cargo.toml

# puis, à chaque fois
python outils\extraire-casc\paquet-sc2.py
```
...
`monter-voix` décode chaque `.ogg` (symphonia, en Rust pur), replie en mono, coupe les silences
...
- **Le paquet pèse 51 Mo** parce que les références sont en 44,1 kHz. Ventriloque ramène tout en
  ...
  d'un rééchantillonneur dans `monter-voix`.
